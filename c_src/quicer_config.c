               ...
               ERL_NIF_TERM elevel)
{
  ...
  status
      = MsQuic->GetParam(s_ctx->Stream, Param, &BufferLength, Buffer);
                   ...
                   ERL_NIF_TERM elevel)
{
  ...
  */
  ...
  status = MsQuic->GetParam(
      c_ctx->Connection, Param, &BufferLength, Buffer);
                 ...
                 ERL_NIF_TERM elevel)
{
  ...
  assert(Param);
  status = MsQuic->SetParam(
      l_ctx->Listener, Param, BufferLength, Buffer);
...
get_tls_opt(ErlNifEnv *env, HQUIC Hanlder, ERL_NIF_TERM optname)
{
  ...
  assert(Param);
  status = MsQuic->GetParam(
      Hanlder, Param, &BufferLength, Buffer);
  if (QUIC_SUCCEEDED(status))
    {
      res = encode_parm_to_eterm(
          env, Param, BufferLength, Buffer);
