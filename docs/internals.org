...
1. **quicer**
   ...
   First entry for developers, provides APIs.
...
3. **quicer_connection**
   ...
   QUIC Connection behaviour

4. **quicer_conn_acceptor_sup**

   QUIC connections sup.
...
7. **quicer_listener_sup**
   QUIC Listeners supervisor

8. **quicer_local_stream**

   Declare QUIC local stream behaviour

   Local Stream is the stream initiated from local

9. **quicer_remote_stream**

   Declare QUIC remote stream behaviour

   Remote Stream is the stream initiated from remote that is accepted locally.

10. **quicer_nif**

    NIF module, backed by C code.

11. **quicer_server_conn_callback**

    Sample implementation of server connecion callbacks.

12. *quicer_stream*

    QUIC stream behaviour.
    For both local and remote streams.
...
| QUIC_CONNECTION_EVENT_PEER_STREAM_STARTED             | {quic, new_stream, stream_handler(), new_stream_props()} stream            |
